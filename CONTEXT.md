...
| 其他后端模块 | 未开始 | — | WorkflowModule（DRAFTING Phase）/ StepModule（IDEA Phase）/ ReviewModule / ChangeAnalysisService / FactSheetCompensationService |
...
| 测试基础设施 | 已完成 (#4, #5, #7, #6, #8, #9, #10, #11) | 后端：Jest 30 + supertest（11 套件 163 条）。前端：Vitest 2.1 + @vue/test-utils + happy-dom（20 套件 212 条） | TDD 红→绿→重构完成。后端 163 条 + 前端 212 条 = 共 375 条 |
