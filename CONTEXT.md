...
| 前端工作流容器（#8） | 已完成 (#8) | `main.ts`（App 入口：createApp → Pinia → Router → `#app` 挂载） `App.vue`（AppLayout + CanvasBackground + `<router-view />`） `router/index.ts`（Vue Router 4 配置：`/` → ProjectHubView，`/project/:id` → WorkflowView） `ProjectHubView.vue`（仪表盘：我的项目标题 + 创建按钮 + 项目列表 + 空状态提示） `WorkflowView.vue`（工作流容器：WorkflowStepper 集成 + 项目标题/ID 展示 + 子路由 `<router-view />` 占位） | TDD 红→绿→重构完成：4 套件 19 条测试通过。Vue Router 4 路由已配置，`useRoute()` 动态读取 `:id` 参数。WorkflowView 通过 `useWorkflowStore` 驱动 WorkflowStepper 的 Phase 状态与步骤交互 |
| 测试基础设施 | 已完成 (#4, #5, #7, #6, #8) | 后端：Jest 30 + supertest（单元 77 条 + 集成 5 条 + e2e 14 条）。前端：Vitest 2.1 + @vue/test-utils + happy-dom（Store 20 条 + 组件 51 条） | TDD 红→绿→重构完成。后端 96 条 + 前端 71 条 = 共 167 条 |
