...
> 最后更新：2026-06-02（修复设定阶段导航回退 Bug + IdeaView 迁移至 `usePhaseWorkflow` 共享逻辑。）
...
| 前端基础设施（#6） | 已完成 (#6) | `useProjectStore`/`useWorkflowStore`/`useBeatStore`/`useChapterStore`（4 个 Pinia store，统一使用 `createStorePersistence` 工厂消除样板代码）。`usePhaseWorkflow` composable（共享 Phase 工作流逻辑：挂载时自动 fetch 后端数据→同步 stepStatus 到 store→提供 `handleGenerate/Regenerate/Confirm/Reject` 统一方法，IdeaView/SettingView/OutlineView 三个视图统一使用）。`useIdeaParser` composable（卖点方案与简介解析逻辑，优先从 `review` 字段读取、`#+` 正则回退解析 `output` 兼容旧数据）。`useContentParser` composable（世界观/角色/关系解析）。`useChapterStore.updateChapterContent` SSE 流式写入 localStorage 限流 300ms（~6000 次/章→~20 次/章）。`sortByChapter` 提取到 `stores/sort.ts` 共享工具。`api/common.ts` 统一 `StepDataResponse` 类型 + `getOrNull` 404 辅助。`<AppLayout />`（a-layout）/`<WorkflowStepper />`（a-steps）/`<CanvasBackground />`（canvas 保留）/`<ModelBadge />`（a-tag）/`<ErrorModal />`（a-modal） | TDD 三轮完成。设计系统 34 个 CSS 自定义属性（teal 绿 `#0d9488` + 琥珀橙 `#f59e0b` + slate 中性色系）+ ConfigProvider 主题 Token 映射，全局注入于 `main.ts`。 |
