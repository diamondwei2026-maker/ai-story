...
> 最后更新：2026-06-05（Issue #26 BEATS 交互重构完成）
...
| StepModule（BEATS Phase #11, #24, #26） | 已完成 (#11, #24, #26) | `BeatsController`（继承 `PhaseControllerBase`——generate/confirm/reject/GET 统一路由模式）+ `BeatModificationController`（全局 Beat 修改路由）。`POST /projects/:id/steps/beats/generate`（新增 `targetChapterCount` 参数）+ `POST /projects/:id/steps/beats/confirm` + `POST /projects/:id/steps/beats/reject` + `GET /projects/:id/steps/beats` + `PATCH /beats/:id/wordcount` + `PATCH /beats/:id/structure` + `POST /:beatId/adjust`（#26 单章自然语言调整）+ `POST /batch-adjust`（#26 区间批量优化）。后端 `step.service.ts`：`parseBeatsFromOutput()` V2 字段解析 + `toBeat()` V1→V2 降级 + `detectImpact()` 钩子链差分检测。前端单列叙事流布局：「BeatsConfigPanel（a-segmented 篇幅+字数+大纲预览）→ BeatsStatsBar（紧凑统计条）→ BeatsRhythmChart（冲突强度+期待值双折线+预警+一键优化）→ BeatNarrativeCard 列表（第一层叙事摘要+换一种写法/第二层手风琴展开冲突描述+钩子因果链）」+ `useBeatsAdjustment` composable（三种调整操作状态管理+影响检测）+ `useBeatStore.ts` + `api/beats.ts`（含 `adjustBeat`/`batchAdjustBeats`）。已删除旧组件 BeatList/BeatEditor/BeatChapterCard/BeatsSummaryCard。HookDensityChart 代码保留但脱离 BeatsView DOM。 | TDD 三阶段完成。#26 状态机：CONFIG→GENERATING→REVIEWING→CONFIRMED（无自动生成，用户手动"开始拆解"）。轻量修改不触发跨 Phase 回退（ADR-0005/0008）。Beat V2 6 新列向下兼容——`plan` 列保留不写入新数据。前端 136 测试（7 新文件）+ 后端 41 Issue #26 专项测试全绿。 |
