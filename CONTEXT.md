...
> 最后更新：2026-05-28（前端 ant-design-vue 全量迁移：所有页面和组件的原生 HTML 元素替换为 ant-design-vue 4.x 组件，ConfigProvider 主题映射 teal 主色，删除 edit-card.css 等冗余样式，416 条测试零回归）
...
| 设计系统刷新 | 已完成 | `css-variables.css` 34 个自定义属性：品牌主色 teal 绿（`--color-primary: #0d9488`，传达健康/成长/创意）、强调色琥珀橙（`--color-accent: #f59e0b`，积极/能量）、中性色系 slate（背景 `#f8fafc`，文字 `#1e293b`/`#64748b`）、状态色（success/warning/error/info + light 变体）、3 级阴影、3 级圆角、2 级过渡时长。全局字体 Google Fonts Inter + Noto Sans SC（抗锯齿渲染）。ant-design-vue 4.x ConfigProvider 全局主题 Token 映射（colorPrimary/colorSuccess/colorWarning/colorError/colorInfo/colorBorder/borderRadius）。CanvasBackground 浮字粒子改 teal 色 `rgba(13, 148, 136, opacity)`。构建修复：`main.ts` 全局 antd 注册 `app.use(Antd)` 替代逐个组件导入，`antd-theme.ts` 独立主题配置。 | 全量 ant-design-vue 迁移完成，416 条测试零回归。 |
