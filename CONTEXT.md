...
| StepModule（BEATS Phase #11, #24, #26） | 已完成 (#11, #24, #26) | `BeatsController`（继承 `PhaseControllerBase`——generate/confirm/reject/GET 统一路由模式）+ `BeatModificationController`（全局 Beat 修改路由）。`POST /projects/:id/steps/beats/generate`（新增 `targetChapterCount` 参数，>35 自动路由 V3 + max_tokens=32768）+ `POST /projects/:id/steps/beats/confirm` + `POST /projects/:id/steps/beats/reject` + `GET /projects/:id/steps/beats` + `PATCH /beats/:id/wordcount` + `PATCH /beats/:id/structure` + `POST /:beatId/adjust`（#26 单章自然语言调整）+ `POST /batch-adjust`（#26 区间批量优化）。后端 `step.service.ts`：`generateBeats()` 高篇幅自适应策略（≤35 R1 / >35 V3）+ `parseBeatsFromOutput()` V2 字段解析 + `toBeat()` V1→V2 降级 + `detectImpact()` 钩子链差分检测。前端单列叙事流布局：「BeatsConfigPanel（a-segmented 篇幅三档+自定义...输入框+字数四档+大纲预览）→ BeatsStatsBar（紧凑统计条）→ BeatsRhythmChart（冲突强度+期待值双折线+预警+一键优化）→ BeatNarrativeCard 列表（第一层叙事摘要+换一种写法/第二层手风琴展开冲突描述+钩子因果链）」+ `useBeatsAdjustment` composable（三种调整操作状态管理+影响检测）+ `useBeatStore.ts` + `api/beats.ts`（含 `adjustBeat`/`batchAdjustBeats`）。AI Gateway 层：`IChatModel.stream()` 支持 `maxTokens` 参数 + `TASK_MAX_TOKENS` 任务级配置 + `callWithFallback()` 支持 `modelOverride`。已删除旧组件 BeatList/BeatEditor/BeatChapterCard/BeatsSummaryCard。HookDensityChart 代码保留但脱离 BeatsView DOM。 | TDD 三阶段完成。#26 状态机：CONFIG→GENERATING→REVIEWING→CONFIRMED（无自动生成，用户手动"开始拆解"）。轻量修改不触发跨 Phase 回退（ADR-0005/0008）。高篇幅自适应策略（今实施）：章数 ≤35→R1，36-90→V3+32K tokens。Beat V2 6 新列向下兼容——`plan` 列保留不写入新数据。前端 141 测试（含 #26 136 + 自定义输入 5）+ 后端 41 Issue #26 专项测试 + 12 AI Gateway 自适应策略测试全绿。 |
| StepModule（DRAFTING Phase #12） | 已完成 (#12, #23) | `POST /projects/:id/chapters/:chapterId/generate`（五步管道：SSE流式正文→指纹提取→FactSheet 乐观锁增量更新→独立审核→摘要，含顺序锁）+ `POST /projects/:id/chapters/:chapterId/confirm`/`dispute`/`pause`/`continue`/`retry` + `GET /projects/:id/chapters`。前端 `StreamingEditor.vue` + `ModeSwitcher.vue` + `TextToolbar.vue` + `ReviewPanel.vue` + `AppealModal.vue` + `DisputeConfirm.vue` + `EditorWorkspace.vue`（#23 编排组件，集成全部 6 子组件）+ `DraftingView.vue`（#22-23，章节列表 + 点击进入 EditorWorkspace 工作区）+ `useChapterStore.ts`（Pinia store，使用 `createStorePersistence` 工厂 + SSE 流式写入 localStorage 300ms 限流） | TDD 红→绿→重构完成。五步管道顺序执行。FactSheet 管线使用 `FactsheetService` CAS 乐观锁。Review 委托 `ReviewService`。EditorWorkspace 编排 6 子组件统一工作区（51 测试），DraftingView 集成（14 新增测试）。 |
...
| 前端基础设施（#6） | 已完成 (#6) | `useProjectStore`/`useWorkflowStore`/`useBeatStore`/`useChapterStore`（4 个 Pinia store，统一使用 `createStorePersistence` 工厂消除样板代码；`useWorkflowStore` 已升级为**项目作用域持久化**——`workflowStore:{projectId}` 按项目隔离，`loadForProject(id)` 加载/切换状态快照，自动迁移旧全局 key）。`usePhaseWorkflow` composable（共享 Phase 工作流逻辑：挂载时自动 fetch 后端数据→同步 stepStatus 到 store→提供 `handleGenerate/Regenerate/Confirm/Reject` 统一方法，IdeaView/SettingView/OutlineView 三个视图统一使用；失败时回退 IN_PROGRESS 防卡死；`previousPhase`/`previousGetFn` 可选参数实现前置阶段状态同步）。`useIdeaParser` composable（卖点方案与简介解析逻辑，优先从 `review` 字段读取、`#+` 正则回退解析 `output` 兼容旧数据）。`useContentParser` composable（世界观/角色/关系解析）。`useChapterStore.updateChapterContent` SSE 流式写入 localStorage 限流 300ms（~6000 次/章→~20 次/章）。`sortByChapter` 提取到 `stores/sort.ts` 共享工具。`api/common.ts` 统一 `StepDataResponse` 类型 + `getOrNull` 404 辅助。`<AppLayout />`（a-layout）/`<WorkflowStepper />`（a-steps，`isClickable` 支持前置全确认时点击后续步骤）/`<CanvasBackground />`（canvas 保留）/`<ModelBadge />`（a-tag）/`<ErrorModal />`（a-modal）/`<MarkdownRenderer />`（marked + DOMPurify GFM 渲染） | TDD 三轮完成。设计系统 34 个 CSS 自定义属性（teal 绿 `#0d9488` + 琥珀橙 `#f59e0b` + slate 中性色系）+ ConfigProvider 主题 Token 映射，全局注入于 `main.ts`。 |
...
| 前端工作流容器 | 已完成 (#7) | `main.ts`（App 入口：createApp → Pinia → Router → `#app` 挂载） `App.vue`（AppLayout + CanvasBackground + `<router-view />`） `router/index.ts`（Vue Router 4 配置：`/` → ProjectHubView，`/project/:id` → WorkflowView，`/project/:id/idea` → IdeaView 子路由，`/project/:id/setting` → SettingView 子路由，`/project/:id/outline` → OutlineView 子路由） `ProjectHubView.vue`（仪表盘：我的项目标题 + 创建按钮 + 项目列表 + 空状态提示；`openProject` 根据项目 `status` 映射到正确 Phase 路由——IDEA→idea / SETTING→setting / OUTLINE→outline，BEATS/DRAFTING/COMPLETED 暂回退到 OUTLINE（**注意**：BEATS/DRAFTING 视图已上线（#11、#22），此回退仅对 COMPLETED 保留；移除需同步 `WorkflowView.syncedPhase` 逻辑）） `WorkflowView.vue`（工作流容器：WorkflowStepper 集成 + 项目标题/ID 展示 + 子路由 `<router-view />` 占位；`handleStepClick` 同时导航路由+更新 store；`watch(projectId)` 调用 `store.loadForProject(id)` 加载项目作用域状态；`watch(projectStore.projects, projectId)` 同步 `store.currentPhase` 与后端 `project.status`） | TDD 红→绿→重构完成。Vue Router 4 路由已配置，`useRoute()` 动态读取 `:id` 参数。WorkflowView 通过 `useWorkflowStore` 驱动 WorkflowStepper 的 Phase 状态与步骤交互 |
| 测试基础设施 | 已完成 (#4–#26) | 后端：Jest 30 + supertest — 20+ 套件 460+ 条。前端：Vitest 2.1 + @vue/test-utils + happy-dom — 48+ 套件 730 条（#26 新增 7 个测试文件 136 测试）。 | TDD 红→绿→重构完成。前后端合计 ~1190 条测试 |
...
- 后端测试：Jest 30 + supertest + RxJS Observable（442 测试）
- 前端测试：Vitest 2.1 + @vue/test-utils + happy-dom（673 测试）
- 运行时：Node.js 22.16
