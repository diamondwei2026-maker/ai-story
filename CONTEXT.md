...
> 最后更新：2026-05-27（Issue #18 关键章节模型路由 + #19 模型降级单向链完成后更新）
...
| StepModule（SETTING Phase #9） | 已完成 (#9) | `POST /projects/:id/steps/setting/generate`（生成世界观设定，含 `currentContent` regenerate 能力）+ `POST /projects/:id/steps/setting/confirm`（确认设定，Project→OUTLINE）+ 前端 `SettingView.vue`（AI 生成流式展示 + 手动 regenerate 按钮 + 确认/拒绝操作）+ `WorldBuilder.vue`（4维度：时代背景/地理环境/社会结构/力量体系，编辑/保存模式）+ `CharacterCard.vue`（3角色：主角/反派/重要配角，每人3弧线：欲望/动机/结局）+ `RelationGraph.vue`（冲突关系 + 情感纽带，编辑/保存模式）+ 共享组件样式 `edit-card.css` + `useContentParser.ts` composable + 路由 `/project/:id/setting` | TDD 红→绿→重构完成。Regenerate 流程：前端传入 `currentContent` → 后端拼入 Prompt 模板 `{{currentContent}}` 变量 → AI 基于用户编辑刷新设定。力量体系红线检查：`runPowerSystemCheck()` 关键词匹配 3 类违规（暴力至上/邪术修炼/贬低制度）。重构阶段提取通用方法 `getStepByProjectId` / `collectAiOutput` / `confirmPhase` |
| StepModule（OUTLINE Phase #10） | 已完成 (#10) | `POST /projects/:id/steps/outline/generate`（生成剧情大纲，参数：`setting`/`structure`/`currentContent`，默认三幕式）+ `POST /projects/:id/steps/outline/confirm`（确认大纲，Project→BEATS）+ `POST /projects/:id/steps/outline/switch-structure`（切换叙事结构，保留关键情节点种子，切换前 `confirm` 提示「关键情节点保留但节点间衔接将被重写」）+ `GET /projects/:id/steps/outline`（获取大纲）+ 前端 `OutlineView.vue`（`<StructureSwitcher />` 三幕式/网文十章/四幕八段选择 + `<OutlineTree />` 大纲树 + `<EmotionCurve />` 情绪曲线标注爽点/虐点/悬念点）+ `usePhaseWorkflow.ts` composable（SettingView/OutlineView 共享状态管理模式）+ 路由 `/project/:id/outline` | TDD 红→绿→重构完成。内嵌审核含节奏评估（`runPacingReview` 多幕/章结构检测）/ 冲突评估（`runConflictReview` 冲突要素检测）/ 高潮评估（`runClimaxReview` 高潮点检测）。重构阶段提取 `api/common.ts` 共享 request 函数、`usePhaseWorkflow` composable 复用 Phase 视图逻辑 |
| StepModule（BEATS Phase #11） | 已完成 (#11) | `POST /projects/:id/steps/beats/generate`（AI 将大纲拆解为逐章 Beat，含冲突点/钩子预设/读者期待值/目标字数，AI 自动提取 hookCount，复用已有 ID 增量更新）+ `POST /projects/:id/steps/beats/confirm`（确认→批量计算 useR1（结构位置开篇1-3/结局末3 或 hookCount≥3 或 isClimax）并固化+创建空壳 Chapter）+ `POST /projects/:id/steps/beats/reject`（驳回→回到 PENDING）+ `GET /projects/:id/steps/beats`（获取 Beat 列表）+ `PATCH /beats/:id/wordcount`（轻量修改一档：仅改字数→该 Beat STALE，不触发跨 Phase 回退）+ `PATCH /beats/:id/structure`（轻量修改二档：改结构内容→该 Beat+对应 Chapter STALE，不触发跨 Phase 回退）+ 前端 `BeatList.vue`（Beat 列表，展示章节号/字数/钩子数/高潮标记/R1标记/STALE 标记）+ `BeatEditor.vue`（双模式编辑器：字数模式/结构模式，isClimax 勾选框+hookCount 只读展示）+ `HookDensityChart.vue`（钩子密度柱状图，≥3 高亮金色）+ `useBeatStore.ts`（Pinia store，含 sortedBeats getter + updateBeatWordCount/updateBeatStructure actions + localStorage 持久化）+ 两个 Controller（`BeatsController` 项目级路由 + `BeatModificationController` 全局 Beat 修改路由） | TDD 红→绿→重构完成。两档轻量修改均不触发跨 Phase 回退（ADR-0005 Decision 1）。重构阶段提取 `sortByChapter`、`markStale` 辅助函数。 |
| StepModule（DRAFTING Phase #12） | 已完成 (#12) | `POST /projects/:id/chapters/:chapterId/generate`（五步管道：SSE流式正文→指纹提取→FactSheet乐观锁增量更新→独立审核→摘要，含顺序锁——前一章 COMPLETED/DISPUTED 后解锁）+ `POST /projects/:id/chapters/:chapterId/confirm`（确认→COMPLETED）+ `POST /projects/:id/chapters/:chapterId/dispute`（争议→DISPUTED，解锁下一章）+ `POST /projects/:id/chapters/:chapterId/pause`（暂停保留已生成内容）+ `POST /projects/:id/chapters/:chapterId/continue`（断点续写，含用户编辑内容回传）+ `POST /projects/:id/chapters/:chapterId/retry`（全量重试五步管道，含 feedback 临时注入 Prompt）+ `GET /projects/:id/chapters`（获取章节列表）+ 前端 `StreamingEditor.vue`（流式编辑器，typewriter 逐字效果，可编辑/只读双模式）+ `ModeSwitcher.vue`（三种生成模式切换：新建续写/段落改写/文笔升级）+ `TextToolbar.vue`（暂停/继续/重试操作，重试含反馈输入框）+ `useChapterStore.ts`（Pinia store，含 chapters/token追加/status更新/mode切换，localStorage 持久化） | TDD 红→绿→重构完成。五步管道顺序执行，每步结果持久化后才进入下一步。≥2500 tokens 时自动生成 contextSummary。重构阶段提取 `getChapterOrThrow`/`transitionChapterStatus`/`runPostGenerationPipeline` 三个私有辅助方法消除重复。 |
| StepModule（Review 审核决策 #13） | 已完成 (#13) | `evaluateChapter(chapterId)`（独立 AI 四维审核：POLITICAL_SAFETY/SEXUAL_CONTENT/VIOLENCE/VALUES，返回 ReviewResult 含 verdict+overallScore+issues）+ `appealReview(chapterId, reason)`（上诉二次审核，仅一次机会，降级按新结论/维持原判按 DISPUTED 路径）+ `getAvailableActions(verdict)`（四档判决动作表查询：PASS→无动作自动确认、PASS_WITH_SUGGESTIONS→采纳或忽略、NEEDS_REVISION→采纳/手动修改/上诉、BLOCKED→手动修改/上诉）+ `getActionsForChapter(chapterId)`（章节级便捷封装）+ `forceDisputeChapter(chapterId)`（强制 DISPUTED→解锁下一章，需满足上诉流程完成前置条件）+ 前端 `ReviewPanel.vue`（审核面板：结论 Badge+综合评分+四维评分网格+问题列表+动态操作按钮组）+ `AppealModal.vue`（上诉弹窗：原始结论展示+上诉理由输入+仅一次提示+提交/取消）+ `DisputeConfirm.vue`（风险确认弹窗：红色警示+强制 DISPUTED 说明+确认/取消） | TDD 红→绿→重构完成。后端提取 4 个私有辅助（`parseAiReviewResponse`/`buildReviewResult`/`normalizeVerdict`/`getReviewResultOrThrow`）消除重复。前端提取 `VERDICT_ACTIONS` 查找表 + `pushAppealAction` 辅助。Prompt 模板：`review/independent-review.md`、`review/appeal-review.md`。 |
...
| StepModule（FactSheet Compensation #15） | 已完成 (#15) | `FactsheetCompensationService`（5 个方法：`enqueue`/`consumeQueue`/`forceSync`/`getQueueDepth`/`getAlertLevel`，含三级阈值 NORMAL<5/PRIORITY≥5/WARNING≥10/CRITICAL≥50，`enqueue` 满 50 抛 `FactsheetQueueFullError`；`drainQueue` 按 `queuedAt` 排序 + `entryById` Map 去重 + latest-wins 冲突裁决）；`StepService` 管线集成（`casWriteFactsheet` CAS 乐观锁 + 冲突时自动重试一次 + 失败入队 `applyCompensationQueue`）；`GET /projects/:projectId/factsheet-alert`（返回 `QueueAlert`）+ `POST /projects/:projectId/factsheet-force-sync`（触发 `forceSync` 返回 `ConsumeResult`）；前端 `WorkflowStepper.vue` 告警横幅（WARNING 黄色 / CRITICAL 红色 + 队列深度显示 + 手动同步按钮 emit `force-sync`） | TDD 红→绿→重构完成。39 后端 service 测试 + 6 controller 测试 + 10 前端组件测试。新增 `PendingFactUpdate` 接口于 `project.entity.ts`（`id`/`chapterId`/`chapterNumber`/`entries`/`queuedAt`）。 |
...
| StepModule（Context Budget #17） | 已完成 (#17) | `calculateBudget(gs, gd, local)`（三层注入 3000/2000/3000 + 单层硬上限 + 优先级总预算 8000t 裁剪）；`computeBudget(projectId, chapterId)`（从 Project 数据自动组装三层上下文）；`generateContextSummary(chapterContent)`（>2500t 触发 AI 摘要 400t，含出场角色/关键事件/情感转折）；`trimToBudget(text, maxTokens)`（单层裁剪）；`estimateTokens(text)`（Token 估算 4 chars/token）；第 1 章特殊处理（IDEA 简介替代前一章） | 三层硬上限 + 优先级裁剪策略落地。Token 预计算模块尚用启发式估算，精确 API tokenizer 留待后续。 |
| StepModule（Model Degradation #19） | 已完成 (#19) | `callWithFallback(taskType, prompt)`（单向降级 V3→R1→阻塞 / R1→V3→阻塞，不循环）；`getDegradationLogs()`（降级日志）；`BLOCKING_TASKS` 9 种（生成+审核+创意）+ `SKIP_ON_FAILURE_TASKS` 3 种（指纹/FactSheet/变更分析）；`AiMeta` 从 AIGatewayService → StepService → 前端 API 传播；前端 `<AiUnavailableModal />`（双模型不可用弹窗）+ `<ModelBadge />`（模型状态指示器）+ `useAiStatus` composable | ADR-0007 Decision 4 落地。降级日志内存存储。 |
...
| IDEA Phase | 已完成 (#8) | `POST /projects/:id/steps/idea/generate`（生成 3-5 个差异化卖点方案，含市场匹配度评分+爆款参考，支持 `feedback` 参数重新生成）+ `POST /projects/:id/steps/idea/summary`（选定卖点后生成一句话简介+500字简介）+ `POST /projects/:id/steps/idea/confirm`（确认灵感，Project IDEA→SETTING，存储 `selectedSellPoint`+`customBrief`）+ `POST /projects/:id/steps/idea/reject`（驳回卖点方案）+ `GET /projects/:id/steps/idea`（获取 IDEA 步骤数据）+ 前端 `IdeaView.vue`（灵感提取完整流程编排：创意输入→卖点选择→简介生成→确认/驳回）+ `IdeaInput.vue`（创意文本输入框，字数统计）+ `SellPointSelector.vue`（卖点方案选择器，含反馈输入+重新生成+生成简介按钮）+ `MarketAnalysisCard.vue`（卖点卡片：标题/核心卖点/市场评分/爆款参考/差异化分析，选中高亮）+ `SummaryCard.vue`（简介展示：一句话+500字+自定义编辑）+ `useIdeaParser.ts` composable（卖点/简介 Markdown 解析）+ `api/idea.ts`（5 个 API 函数）+ 路由 `/project/:id/idea` | TDD 红→绿→重构完成。内嵌审核含合规检查（`runPowerSystemCheck` 力量体系红线）+「仅供参考」标注。重构阶段：`confirmIdea` 复用 `confirmPhase` 消除重复；`useIdeaParser` composable 提取解析逻辑 |
| 前端基础设施（#6） | 已完成 (#6) | `useProjectStore`（项目列表+当前项目+localStorage 持久化） `useWorkflowStore`（Phase 管理+步骤状态+Phase 访问控制+localStorage 持久化） `<AppLayout />`（书卷风布局） `<WorkflowStepper />`（5 步进度条：灵感提取→设定集→剧情大纲→细纲拆解→正文迭代，当前步金色高亮，已确认绿色勾，未来步不可点击，步骤计数器） `<CanvasBackground />`（Canvas 粒子动画，默认 50 粒子） `<ModelBadge />`（模型名称绿色 Badge，降级黄色+「已降级至 XX」） `<ErrorModal />`（双模型不可用弹窗，手动重试按钮，遮罩点击/ESC 关闭） | TDD 三轮完成：7 套件 52 条测试通过。持久化采用手动 `loadJSON`/`saveJSON`（`stores/persist.ts`）替代 PRD 规划的 pinia-plugin-persistedstate（避免测试环境额外配置）。CSS 设计 token 统一（`css-variables.css` 7 变量）。Ant Design Vue v4 已安装，当前组件为纯 CSS 实现，后续 Issue 逐步引入 a-steps/a-modal 等 |
...
| 测试基础设施 | 已完成 (#4–#18) | 后端：Jest 30 + supertest — 18 套件 396 条（含 Unit + E2E）。前端：Vitest 2.1 + @vue/test-utils + happy-dom — 37 套件 416 条 | TDD 红→绿→重构完成。前后端合计 812 条测试，零回归 |
