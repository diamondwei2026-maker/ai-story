...
| 日期 | 变更 | 影响范围 |
|------|------|----------|
| 06-04 | **BEATS 可视化组件补齐** — 新建 4 个前端 Vue 组件完善细纲拆解阶段 UI：① `BeatsConfigPanel.vue` — 生成配置面板（默认字数设置 + 重新生成触发），已确认后切换为只读摘要；② `BeatChapterCard.vue` — 章节详情卡片（冲突点摘要 + 钩子标签 + 高潮/R1/STALE 徽章 + 节奏评级），点击选中，替代右侧空态；③ `BeatsRhythmChart.vue` — 叙事节奏曲线图（ECharts 双轴：钩子密度折线 + 字数柱状图，高潮标记点 + R1 阈值线 + R1 章节色标），Canvas 不可用时优雅降级；④ `BeatsSummaryCard.vue` — 细纲统计摘要卡片（总章节/总字数/R1 章节/高潮章 + 平均钩子密度/平均字数/R1 占比/STALE 计数进度条）。`BeatsView.vue` 集成四组件，调整布局为「ConfigPanel → SummaryCard → RhythmChart → HookDensityChart → [BeatList | BeatChapterCard + BeatEditor]」。TDD：4 个组件 39 测试 + BeatsView 回归 13 测试，共 52 测试全绿。新增 npm 依赖 `echarts`。 | BeatsSummaryCard.vue / BeatChapterCard.vue / BeatsConfigPanel.vue / BeatsRhythmChart.vue + 4 个 spec / BeatsView.vue |
...
| StepModule（BEATS Phase #11, #24） | 已完成 (#11, #24) | `BeatsController`（继承 `PhaseControllerBase`——generate/confirm/reject/GET 统一路由模式）+ `BeatModificationController`（全局 Beat 修改路由）。`POST /projects/:id/steps/beats/generate` + `POST /projects/:id/steps/beats/confirm` + `POST /projects/:id/steps/beats/reject` + `GET /projects/:id/steps/beats` + `PATCH /beats/:id/wordcount` + `PATCH /beats/:id/structure`。前端 `BeatList.vue` + `BeatEditor.vue`（两模式切换：字数/结构）+ `BeatChapterCard.vue`（章节详情卡片：冲突点 + 钩子标签 + 高潮/R1/STALE 徽章 + 节奏评级）+ `BeatsConfigPanel.vue`（生成配置：默认字数 + 重新生成触发，已确认后只读）+ `BeatsSummaryCard.vue`（全篇统计：总章节/总字数/R1/高潮 + 平均密度/字数/占比/STALE 进度条）+ `BeatsRhythmChart.vue`（ECharts 双轴节奏曲线：钩子密度折线 + 字数柱状图 + 高潮标记点 + R1 阈值线）+ `HookDensityChart.vue` + `useBeatStore.ts`（Pinia store，使用 `createStorePersistence` 工厂 + 共享 `sortByChapter` 工具）+ `api/beats.ts`（含 `updateBeatWordCount` / `updateBeatStructure` API 函数）+ DraftingView beat 信息展示（冲突点摘要 + 钩子计数 + STALE 徽章）。 | TDD 红→绿→重构完成。两档轻量修改均不触发跨 Phase 回退（ADR-0005 Decision 1）。#24 前端集成：DraftingView 展示 beat 结构摘要 + STALE 标记，api/beats.ts 对接 `PATCH /beats/:id/structure`。四组件补齐 BeatsView 完整可视化：52 测试全绿（BeatsSummaryCard 9 + BeatChapterCard 14 + BeatsConfigPanel 10 + BeatsRhythmChart 6 + BeatsView 13）。 |
...
| 测试基础设施 | 已完成 (#4–#25) | 后端：Jest 30 + supertest — 20 套件 442 条。前端：Vitest 2.1 + @vue/test-utils + happy-dom — 47 套件 673 条（#25 重构 OutlineView.spec.ts：44 测试；BEATS 四组件补齐：BeatsSummaryCard 9 + BeatChapterCard 14 + BeatsConfigPanel 10 + BeatsRhythmChart 6 = 39 新增测试） | TDD 红→绿→重构完成。前后端合计 1115 条测试 |
