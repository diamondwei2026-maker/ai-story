...
| StepModule（SETTING Phase #9） | 已完成 (#9) | `POST /projects/:id/steps/setting/generate`（生成世界观设定，含 `currentContent` regenerate 能力）+ `POST /projects/:id/steps/setting/confirm`（确认设定，Project→OUTLINE）+ 前端 `SettingView.vue`（AI 生成流式展示 + 手动 regenerate 按钮 + 确认/拒绝操作）+ `WorldBuilder.vue`（4维度：时代背景/地理环境/社会结构/力量体系，编辑/保存模式）+ `CharacterCard.vue`（3角色：主角/反派/重要配角，每人3弧线：欲望/动机/结局）+ `RelationGraph.vue`（冲突关系 + 情感纽带，编辑/保存模式）+ 共享组件样式 `edit-card.css` + `useContentParser.ts` composable + 路由 `/project/:id/setting` | TDD 红→绿→重构完成：前端 SettingView 6 条测试 + WorldBuilder 12 条 + CharacterCard 14 条 + RelationGraph 12 条 + Router 2 条，后端 StepService 2 条新测试 + E2E 1 条新测试。Regenerate 流程：前端传入 `currentContent` → 后端拼入 Prompt 模板 `{{currentContent}}` 变量 → AI 基于用户编辑刷新设定。力量体系红线检查：`runPowerSystemCheck()` 关键词匹配 3 类违规（暴力至上/邪术修炼/贬低制度） |
| 其他后端模块 | 未开始 | — | WorkflowModule（IDEA/OUTLINE/BEATS/DRAFTING Phase）/ ReviewModule / ChangeAnalysisService / FactSheetCompensationService |
| IDEA Phase | 未开始 | — | PRD 用户故事 5-8：灵感输入、市场分析、卖点方案选择、一句话/500字简介生成、资深编辑审核。前端需 StepIdea 模块（IdeaInput / MarketAnalysisCard / SellPointSelector）。建议作为 Issue #10 |
