...
| **Step（阶段内步骤）** | 单个 Phase 内部的子状态流转。**后端 StepData.status**（Prisma 实际枚举，5 种）：`PENDING` → `AI_GENERATING`（AI 返回中）→ `AWAITING_REVIEW`（AI 完成，等待用户确认）→ `CONFIRMED` / `REJECTED`（驳回后需重新生成，后台 `generate` 接口自动重置为 `PENDING` 后再次进入 `AI_GENERATING`）。**前端 StepStatus**（派生类型，4 种）：`PENDING`（待开始）/ `IN_PROGRESS`（进行中——合并映射 `AI_GENERATING` + `AWAITING_REVIEW`）/ `CONFIRMED`（已完成）/ `REJECTED`（需修改）。StepData 为独立 MongoDB 文档，通过 `projectId` 关联 Project，记录每个 Phase 的输入、输出、审核结果和版本历史。 |
...
> 最后更新：2026-06-04（Issue #25 大纲叙事结构选择器恢复完成）

### 近期更新（2026-06-01 ~ 2026-06-04）

| 日期 | 变更 | 影响范围 |
|------|------|----------|
| 06-04 | **Issue #25 大纲叙事结构选择器恢复** — ① 恢复 `OutlineView.vue` 中被 `v-if="false"` 隐藏的叙事结构选择器（`a-segmented`），用户可在大纲未确认时自由切换三幕式/网文十段/四幕八段三种结构；② 原生 `window.confirm()` 替换为 Ant Design `<a-modal>` 确认弹窗（`get-container="false"` inline 渲染确保可测试），含确认/取消按钮和"关键情节点保留"警告文案；③ `selectedStructure` 默认值从硬编码 `'web-novel-ten'` 改为从 outline 的 `input` 字段动态解析（`parseStructureFromInput` regex），无现有 outline 时回退 `'three-act'`；④ 新增 `previousStructure` ref 替代取消时的重复 re-parse 逻辑；⑤ 解析器（`parsedOutline` computed regex）已原生支持三种结构的段标记（第X幕/第X段/Act X），无需修改。TDD 三阶段：RED（23 fail 21 pass）→ GREEN（32 pass 12 fail，Issue #25 18 测试全绿）→ REFACTOR（`makeOutlineData` helper + `flushAsync` 统一 wait pattern，消除 ~77% mock 数据重复）。后端 `switch-structure` 端点早已完成（#10），本次纯前端 UI 恢复与管线对接。 | OutlineView.vue + spec / api/outline.ts |
...
| 测试基础设施 | 已完成 (#4–#25) | 后端：Jest 30 + supertest — 20 套件 442 条。前端：Vitest 2.1 + @vue/test-utils + happy-dom — 43 套件 600 条（#25 重构 OutlineView.spec.ts：44 测试覆盖 5 组场景——选择器可见性/默认值解析/确认弹窗/结构解析/切换流程） | TDD 红→绿→重构完成。前后端合计 1042 条测试 |
...
- 前端测试：Vitest 2.1 + @vue/test-utils + happy-dom（600 测试）
- 运行时：Node.js 22.16
