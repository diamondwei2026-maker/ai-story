...
> 最后更新：2026-06-04（Issue #24 BEATS 结构修改前端集成完成）
...
| 测试基础设施 | 已完成 (#4–#24) | 后端：Jest 30 + supertest — 20 套件 446 条（含 Unit + E2E；#24 新增 16 条 `updateBeatStructure` 测试）。前端：Vitest 2.1 + @vue/test-utils + happy-dom — 39 套件 585 条（#24 新增 11 条：6 API `updateBeatStructure`/`updateBeatWordCount` + 5 DraftingView beat 集成） | TDD 红→绿→重构完成。前后端合计 1031 条测试 |
