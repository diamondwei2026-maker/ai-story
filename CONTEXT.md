...
| 其他后端模块 | 未开始 | — | WorkflowModule（BEATS/DRAFTING Phase）/ StepModule（IDEA Phase）/ ReviewModule / ChangeAnalysisService / FactSheetCompensationService |
...
| 测试基础设施 | 已完成 (#4, #5, #7, #6, #8, #9, #10) | 后端：Jest 30 + supertest（单元 106 条 + e2e 36 条）。前端：Vitest 2.1 + @vue/test-utils + happy-dom（16 套件 157 条） | TDD 红→绿→重构完成。后端 142 条 + 前端 157 条 = 共 299 条 |
