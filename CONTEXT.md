...
> 最后更新：2026-06-04（剧情大纲叙事结构默认改为网文十段）
...
| StepModule（BEATS Phase #11, #24） | 已完成 (#11, #24) | `BeatsController`（继承 `PhaseControllerBase`——generate/confirm/reject/GET 统一路由模式）+ `BeatModificationController`（全局 Beat 修改路由）。`POST /projects/:id/steps/beats/generate` + `POST /projects/:id/steps/beats/confirm` + `POST /projects/:id/steps/beats/reject` + `GET /projects/:id/steps/beats` + `PATCH /beats/:id/wordcount` + `PATCH /beats/:id/structure`。前端 `BeatList.vue` + `BeatEditor.vue`（两模式切换：字数/结构）+ `HookDensityChart.vue` + `useBeatStore.ts`（Pinia store，使用 `createStorePersistence` 工厂 + 共享 `sortByChapter` 工具）+ `api/beats.ts`（含 `updateBeatWordCount` / `updateBeatStructure` API 函数）+ DraftingView beat 信息展示（冲突点摘要 + 钩子计数 + STALE 徽章）。 | TDD 红→绿→重构完成。两档轻量修改均不触发跨 Phase 回退（ADR-0005 Decision 1）。#24 前端集成：DraftingView 展示 beat 结构摘要 + STALE 标记，api/beats.ts 对接 `PATCH /beats/:id/structure`。 |
...
| 测试基础设施 | 已完成 (#4–#25) | 后端：Jest 30 + supertest — 20 套件 442 条。前端：Vitest 2.1 + @vue/test-utils + happy-dom — 43 套件 634 条（#25 重构 OutlineView.spec.ts：44 测试覆盖 5 组场景——选择器可见性/默认值解析/确认弹窗/结构解析/切换流程） | TDD 红→绿→重构完成。前后端合计 1076 条测试 |
...
- 前端测试：Vitest 2.1 + @vue/test-utils + happy-dom（634 测试）
- 运行时：Node.js 22.16
