...
**Decision 2 — 模型分级**：章节正文按调用类型和重要性分级使用不同模型。前期 Phase 生成（IDEA/SETTING/OUTLINE/BEATS）和关键章节正文（开篇、高潮、结局等，由 BEATS 钩子密度自动识别）使用 DeepSeek-R1；普通章节正文、正文改写/润色、独立合规审核、指纹/FactSheet 提取等高频任务使用 DeepSeek-V3。模型选择由代码集中映射（ADR-0004），当前不支持用户手动覆盖——钩子密度自动分配已覆盖关键章节识别。
